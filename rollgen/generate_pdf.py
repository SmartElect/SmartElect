...
def generate_pdf(filename, center, voter_roll, gender, center_book=False):
    ...
    for page in chunker(voter_roll, settings.ROLLGEN_REGISTRATIONS_PER_PAGE_REGISTRATION):
        ...
        # The contents of each table cell are wrapped in a Paragraph to set the base text direction.
    ...
    if not n_pages:
        # When there are no pages (==> no registrants for this gender), we need to emit a page that
        # states that.
        elements.append(page_header)
